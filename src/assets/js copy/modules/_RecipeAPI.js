...
class RakutenRecipeAPI {
	...
	*/
	init () {
		...
		*/
		...
		if ((this.data[0] !== undefined)) {
			...
			numberOfRecipes = date > 7 ? 7 : date;
		}

		if (numberOfRecipes > 0) {
			// レシピ取得
			this.getRecipe(numberOfRecipes);
		// } else {
		// 	// 書き出し
	 ...
	 * レシピを取得して書き出し
	 *
	 * @param {number} numberOfRecipes 取得したいレシピ数
	 * @return {Promise}
	*/
	getRecipe (numberOfRecipes) {
		/**
		 * ローカルストレージにデータをセット
		 *
		 * @param {Array} options.dataList 取得したレシピリスト
		 * @param {string} options.numberOfRecipes 取得したいレシピ数
		 * @return {void}
		*/
		const setLocalStorage = (options) => {
			...
			for (let i = 0; i < options.dataList.length - 1; i++) {
				...
				if (i + options.numberOfRecipes < 7) {
					// 差分が6未満なら、データを移行
					this.data[i] = this.data[i + options.numberOfRecipes];
				} else {
					// 差分が6以上なら、新しくデータをセット
					this.data[i] = {
		...
		this.ajaxRecipe().then((data1) => {
			...
			setTimeout(() => {
				this.ajaxRecipe().then((data2) => {
					dataList.push(...data2);
					// データ更新
					setLocalStorage({dataList, numberOfRecipes});
					// 書き出し
					this.updateCassetteContents();
				})
				.catch(() => {
